...
def check_luhn(card, valid):
    ...
    Debug 1
    Possible that the number is not double digits, which would leave 
    certain variables empty. Assigning this to a int will cause issues.

    Debug 2
    Possible error in calculations due to tests with valid cards 
    returning a sum that does not end in 0

    Debug 3
    Do not understand what checksum digit is for. Added to the sum
    like normal.         
    """        
